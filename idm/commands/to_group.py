    ...
    try:
        ...
        if e.error_code == 214:
            utils.new_message(
                event.api,
                event.chat.peer_id,
                message="❗ Ошибка при публикации. Публикация запрещена. "
                        "Превышен лимит на число публикаций в сутки, либо на указанное время уже запланирована другая "
                        "запись, либо для текущего пользователя недоступно размещение записи на этой стене."
            )
        elif e.error_code == 220:
            utils.new_message(
                event.api, event.chat.peer_id, message="❗ Ошибка при публикации. Слишком много получателей.")
        elif e.error_code == 222:
            utils.new_message(
                event.api, event.chat.peer_id, message="❗ Ошибка при публикации.Запрещено размещать ссылки.")
        else:
            utils.new_message(
                event.api, event.chat.peer_id, message=f"❗ Ошибка при публикации. Ошибка VK: {e.error_msg}.")
    except:

...
def remove_from_fr(event: MySignalEvent) -> str:
    ...
    try:
        ...
        if data.get('friend_deleted', False):
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="✅ Друг удален")
        elif data.get('out_request_deleted', False):
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="✅ Отменена исходящая заявка")
        elif data.get('in_request_deleted', False):
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="✅ Отклонена входящая заявка")
        elif data.get('suggestion_deleted', False):
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="✅ Отклонена рекомендация друга")
        elif data.get('success', False):
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="✅ Друг удален")
        else:
            edit_message(event.api, event.chat.peer_id, event.msg['id'], message="❗ Произошла ошибка")
    except VkApiResponseException as e:
        edit_message(event.api, event.chat.peer_id, event.msg['id'],
                     message=f"❗ Произошла ошибка VK №{e.error_code} {e.error_msg}")

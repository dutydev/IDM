...
def delete_messages_from_user(event: Event) -> str:
    ...
    msg_ids = []
    for message in utils.get_all_history_gen(event.api, event.chat.peer_id):
        if datetime.now().timestamp() - message['date'] >= 86400:
            break
        if message['from_id'] == user_id and message.get('action', None) is None:
    ...
    try:
        event.api("messages.delete", message_ids=",".join(msg_ids), delete_for_all=1,
                  spam=1 if event.obj.get("is_spam", False) else 0)
        ...
        utils.edit_message(
            ...
            event.chat.peer_id,
            message_id,
            message=f"❗ Произошла неизвестная ошибка."
        )
    return "ok"

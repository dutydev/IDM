...
class Event:
    ...
    def __init__(self, request: Request):
        ...
        if request.data == b'':
            ...
            if 'chat' in self.obj.keys() and self.msg != None:
                if self.obj['chat'] not in self.db.chats.keys():
                    chats = self.api("messages.getConversations", count=100, filter="all")
                    for item in chats['items']:
                        conver = item['conversation']
                        if conver['peer']['type'] == "chat":
                            message = utils.get_msg(self.api, conver['peer']['id'], self.msg['conversation_message_id'])
                            ...
                            if message['from_id'] == self.msg['from_id'] and message['date'] == self.msg['date']:
                                ...
                                self.db.save()
                                break
                self.chat = Chat(self.db.chats[self.obj['chat']], self.obj['chat'], self.obj['chat'])
                self.message = self.msg = utils.get_msg(self.api, self.chat.peer_id, 
